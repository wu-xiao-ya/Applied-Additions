...
public class AAECraftingUnitModelProvider {
    ...
    public static Collection<ResourceLocation> getTextures() {
        return Arrays.asList(STRUCTURE_FORMED_FACE, STRUCTURE_FORMED_SIDES, STRUCTURE_ANIMATION_SIDES, INTERNAL_FORMED_FACE, INTERNAL_FORMED_SIDES, INTERNAL_ANIMATION_SIDES, INTERNAL_ANIMATION_FACE, INTERNAL_ANIMATION_FACE_TB);
    }
    ...
    public IBakedModel bake(VertexFormat format, Function<ResourceLocation, TextureAtlasSprite> textureGetter) {
        if (this.type == AAECraftingUnitType.QUANTUM_STRUCTURE) {
            return QuantumComputerBakedModel.structure(format, textureGetter.apply(STRUCTURE_FORMED_FACE), textureGetter.apply(STRUCTURE_FORMED_SIDES), textureGetter.apply(STRUCTURE_ANIMATION_SIDES));
        }

        return QuantumComputerBakedModel.internal(format, textureGetter.apply(INTERNAL_FORMED_FACE), textureGetter.apply(INTERNAL_FORMED_SIDES), textureGetter.apply(INTERNAL_ANIMATION_SIDES), textureGetter.apply(INTERNAL_ANIMATION_FACE), textureGetter.apply(INTERNAL_ANIMATION_FACE_TB), textureGetter.apply(INTERNAL_ANIMATION_FACE_TB));
    }
}
